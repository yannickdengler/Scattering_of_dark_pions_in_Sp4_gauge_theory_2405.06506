...
sigma_v_data = np.transpose(np.genfromtxt("output/tables/sigma_v_data_full.csv",delimiter=","))
...
# print(len(LS_data_in))

# print(len(sigma_v_data))

for i in range(len(Sp4_data_in)):
    plt.plot(varr, Sp4_data_in[i],color="orange")

varr = np.insert(varr,0,10)
# LS_data = np.insert(LS_data_in,0,10)
...
plt.plot(varr[1:],LS_data_in,color="black", label = "Ligma")
